...
function rollDice() {
  ...
  setTimeout(() => {
    ...
    if (sum % 2 === 0) {
      ...
      resultMessageElement.textContent = `It's even, you need to add.`;
      randomNumberElement.textContent = `Looks like you need to make ${randomAdd}.`;
    } else {
      // Generate a random number for subtraction between 0 and 5
      const randomSubtract = Math.floor(Math.random() * 6);
      resultMessageElement.textContent = `It's odd, you need to subtract.`;
      randomNumberElement.textContent = `Looks like you need to make ${randomSubtract}.`;
    }
